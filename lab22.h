...
void Unit::equip(Equipment *sth){
	hpmax = b_hpmax + sth->getStat()[0];
	atk = b_atk + sth->getStat()[1];
	def = b_def + sth->getStat()[2];
...
int Unit::beAttacked(int oppatk){
	...
	if(oppatk > def){
		...
		t = 1 + rand()%100;
		if(dodge_on && t > 50) dmg = 0;
		else if(dodge_on && t <= 50) dmg = dmg * 2;
	}	
	hp -= dmg;
	if(hp <= 0){hp = 0;}
